...
const Receipt = forwardRef<HTMLDivElement, ReceiptProps>(({ transaction, items, admin, user, customerName, customerAddress, customerPhone, adminName }, ref) => {
  ...
  const CustomerCopy = (
    <div style={{ width: '100%', maxWidth: 'none' }}>
      <div className="text-[10px] uppercase tracking-wider mb-1 text-right font-bold text-black">Customer Copy</div>
      <div className="bg-white text-black p-2 border border-blue-700 receipt-container" style={{ minHeight: "100%", width: '100%', maxWidth: 'none' }}>
        <div className="pb-1 mb-1 border-b-2 border-blue-700">
          {/* Header: Logo Left, Text Center */}
          <div className="flex items-center gap-2 mb-1">
            <div className="w-14 h-14 shrink-0 rounded-full overflow-hidden border border-blue-700">
              <img src="/logo.jpg" alt="Logo" className="w-full h-full object-cover" />
            </div>
            <div className="flex-1 flex flex-col items-center text-center">
              <div className="text-3xl font-black tracking-wide text-blue-700 leading-none mb-0.5">MARSHALL ETHEL NIG. LTD.</div>
              <div className="text-[9px] leading-3 bg-red-700 text-white px-2 py-0.5 rounded max-w-[95%] mx-auto">
                Dealers on Electronics/Electrical such as: Fridge, TV sets, Plasma, CDs, VCDs, Air conditioners, Generating Sets, Electric Wires, Accessories fittings, etc.
              </div>
              <div className="mt-0.5">
                <div className="inline-block px-3 py-0 rounded-full border border-red-700 text-red-700 font-bold text-[10px] leading-tight">
                  CASH/CREDIT SALES INVOICE
                </div>
              </div>
            </div>
            {/* Spacer to balance logo for perfect centering of text */}
            <div className="w-14 shrink-0"></div>
          </div>

          {/* 3-Column Layout: Head Office | Logos | Branch Office */}
          <div className="grid grid-cols-[150px_1fr_150px] gap-1 items-start text-[9px] branch-grid mt-1 leading-tight">
            {/* Left: Head Office */}
            <div className="text-left">
              <div className="font-bold text-blue-700 text-[10px]">HEAD OFFICE</div>
              <div>No. 10 Oko Road</div>
              <div>Ekwulobia, Aguata L.G.A</div>
              <div className="font-bold">08069818905</div>
            </div>

            {/* Center: Logos (3 lines) */}
            <div className="flex flex-col items-center justify-center gap-0.5 px-0.5">
              <div className="flex gap-1.5">
                <SafeImg src="/lg-logo.jpg" alt="LG" className="h-4 w-auto object-contain" />
                <SafeImg src="/pe-logo.jpg" alt="PE" className="h-4 w-auto object-contain" />
                <SafeImg src="/htcool-logo.jpg" alt="HTC" className="h-4 w-auto object-contain" />
                <SafeImg src="/samsung-logo.jpg" alt="Sam" className="h-4 w-auto object-contain" />
              </div>
              <div className="flex gap-1.5">
                <SafeImg src="/panasonic-logo.jpg" alt="Pan" className="h-4 w-auto object-contain" />
                <SafeImg src="/hisense-logo.jpg" alt="His" className="h-4 w-auto object-contain" />
                <SafeImg src="/binatone.png" alt="Bin" className="h-4 w-auto object-contain" />
                <SafeImg src="/nexus.png" alt="Nex" className="h-4 w-auto object-contain" />
              </div>
              <div className="flex gap-1.5">
                <SafeImg src="/scanfrost.png" alt="Sca" className="h-4 w-auto object-contain" />
                <SafeImg src="/skyrun.png" alt="Sky" className="h-4 w-auto object-contain" />
                <SafeImg src="/phillips.png" alt="Phi" className="h-4 w-auto object-contain" />
              </div>
            </div>

            {/* Right: Branch Office */}
            <div className="text-right">
              <div className="font-bold text-blue-700 text-[10px]">BRANCH OFFICE</div>
              <div>No. 9 Oko Road</div>
              <div>Ekwulobia, Aguata L.G.A</div>
              <div className="font-bold break-words">08130087873, 09122430843</div>
            </div>
          </div>
        </div>

        {/* Responsive grid for customer/transaction details */}
        {/* Responsive grid for customer/transaction details - Static 2-col for printer */}
        <div className="border border-blue-200 rounded bg-blue-50/50 p-1 mb-1 text-[9px] w-full grid grid-cols-2 gap-2 page-keep details-grid leading-snug">
          <div className="min-w-[180px]">
            <div className="flex gap-1"><span className="font-bold text-blue-900 w-20 shrink-0">Customer Name:</span> <span className="font-medium truncate">{customerName || "-"}</span></div>
            <div className="flex gap-1"><span className="font-bold text-blue-900 w-20 shrink-0">Phone:</span> <span className="font-medium truncate">{customerPhone || "-"}</span></div>
            <div className="flex gap-1"><span className="font-bold text-blue-900 w-20 shrink-0">Address:</span> <span className="font-medium truncate">{customerAddress || "-"}</span></div>
          </div>
          <div className="min-w-[180px] md:text-right">
            <div className="flex gap-1 justify-start md:justify-end"><span className="font-bold text-blue-900">Date:</span> <span className="font-medium">{dateStr}</span></div>
            <div className="flex gap-1 justify-start md:justify-end"><span className="font-bold text-blue-900">Time:</span> <span className="font-medium">{timeStr}</span></div>
            <div className="flex gap-1 justify-start md:justify-end"><span className="font-bold text-blue-900">Handled By:</span> <span className="font-medium">{adminName || admin?.full_name || user?.email || "-"}</span></div>
          </div>
        </div>
  ...
  const ShopOwnerCopy = (
    <div style={{ width: '100%', maxWidth: 'none' }}>
      <div className="text-[10px] uppercase tracking-wider mb-1 text-right font-bold text-black border-t-2 border-dashed border-gray-400 pt-2 mt-2">Shop Owner Copy</div>
      <div className="bg-white text-black p-2 border border-blue-700 receipt-container" style={{ minHeight: "100%", width: '100%', maxWidth: 'none' }}>
        <div className="border border-blue-200 rounded bg-blue-50/50 p-1 mb-1 text-[9px] w-full grid grid-cols-2 gap-2 page-keep details-grid leading-snug">
          <div className="min-w-[180px]">
            <div className="flex gap-1"><span className="font-bold text-blue-900 w-20 shrink-0">Customer Name:</span> <span className="font-medium truncate">{customerName}</span></div>
    ...
    <div ref={ref} className={`bg-white text-black p-3 sm:p-4 w-full mx-auto receipt-print density-${density}`} style={{ minHeight: "auto", maxWidth: 'none' }}>
      <style>{`
        ...
        @media print {
          ...
          .receipt-container { max-width: none; width: 100%; border: 1px solid #1d4ed8 !important; }
          ...
          /* Enforce static 2-column grid for details */
          .details-grid { display: grid !important; grid-template-columns: 1fr 1fr !important; column-gap: 8px; align-items: start; }
          
          /* Enforce static 3-column grid for branch/logos */
          .branch-grid { display: grid !important; grid-template-columns: 150px 1fr 150px !important; align-items: start; }
          
          table { border-collapse: collapse; table-layout: fixed; width: 100%; }
          table, tr, td, th { page-break-inside: avoid; border-color: #1d4ed8 !important; }
          ...
          .receipt-copy { page-break-inside: avoid; margin-bottom: 20px; }
